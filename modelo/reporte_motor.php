...
if ($FECHA_FIN or $SCHASIS) {
    ...
    $query = ("begin zrxi.rpv_serie_motor_nac('$SCHASIS'); end;");
    ...
    $query = ("select * from zrxi.ta_Rpv_serie_motor_nac");
    $stid = oci_parse($conn, $query);
    oci_set_prefetch($stid, 60);
    oci_execute($stid);
...
<head>
    ...
    <title>Resultados de la serie / motor: <?php echo $SCHASIS ?></title>
    ...
    <style>
        ...
        #example th:nth-child(2), #example td:nth-child(2) { width: 14%; } /* Oficina */
        #example th:nth-child(3), #example td:nth-child(3) { width: 8%; }  /* Año */
        #example th:nth-child(4), #example td:nth-child(4) { width: 12%; } /* N° de Título */
        #example th:nth-child(5), #example td:nth-child(5) { width: 10%; } /* N° de Placa */
        #example th:nth-child(6), #example td:nth-child(6) { width: 14%; } /* N° de Serie */
        #example th:nth-child(7), #example td:nth-child(7) { width: 14%; } /* N° de Motor */
        #example th:nth-child(8), #example td:nth-child(8) { width: 12%; } /* N° de VIN */
        #example th:nth-child(9), #example td:nth-child(9) { width: 8%; } /* Destino */
        ...
        a[href="../vista/reporte03.php"]:hover {
...
<div class="page-wrapper">
    <!-- Top Bar -->
    <div class="top-bar">
        <a href="../vista/reporte03.php" class="back-button">
    ...
    <div class="header-section">
        ...
        <h1 class="main-title">CONSULTA POR SERIE/MOTOR</h1>
        <p class="subtitle">Sistema de Consulta de Vehículos por Número de Serie y Motor</p>
    </div>
</div>

    <div class="contenedor_tabla">
        <div class="results-header">
            <h2><i class="fas fa-car"></i> Resultados de la serie / motor: <?php echo $SCHASIS ?></h2>
        </div>
                ...
                <table id="example" class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        ...
                        while ($myrow = oci_fetch_array($stid, OCI_ASSOC)) {
                            ...
                            echo "<td><span style='color: var(--sunarp-turquesa); font-weight: 600;'>{$myrow['AN_TITU']}</span></td>";
                            echo "<td><strong>{$myrow['NO_TITU']}</strong></td>";
                            echo "<td><span style='background: var(--sunarp-yellow); color: white; padding: 0.2rem 0.4rem; border-radius: 8px; font-size: 0.7rem; font-weight: 600;'>{$myrow['NO_PLAC']}</span></td>";
                            echo "<td><code style='background: rgba(26, 166, 164, 0.1); color: var(--sunarp-turquesa); padding: 0.2rem 0.4rem; border-radius: 4px; font-size: 0.7rem;'>{$myrow['NO_SERI']}</code></td>";
                            echo "<td><code style='background: rgba(26, 166, 164, 0.1); color: var(--sunarp-turquesa); padding: 0.2rem 0.4rem; border-radius: 4px; font-size: 0.7rem;'>{$myrow['NO_MOTR']}</code></td>";
                            echo "<td><code style='background: rgba(26, 166, 164, 0.1); color: var(--sunarp-turquesa); padding: 0.2rem 0.4rem; border-radius: 4px; font-size: 0.7rem;'>{$myrow['NO_VIN']}</code></td>";
                            echo "<td><span style='color: var(--sunarp-green); font-weight: 600;'>{$myrow['DESTINO']}</span></td>";
                            echo "</tr>";
                        }
                        ?>
                    </tbody>
        ...
        <div class="action-buttons">
            ...
            <button onclick="imprimirTabla()" class="btn-outline-primary">
...
function imprimirTabla() {
    ...
    doc.open();
    doc.write('<html><head><title>Imprimir Reporte Serie/Motor</title>');
    doc.write('<style>');
    ...
    doc.write('<h2>Resultados de Consulta por Serie/Motor: <?php echo $SCHASIS; ?></h2>');
